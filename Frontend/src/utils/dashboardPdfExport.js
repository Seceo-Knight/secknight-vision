...
// report and every other report PDF resolve the SecKnight Vision logo the
...
export const generateModulePdf = async ({
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...BRAND.subtle);
    doc.text("SecKnight Vision • Your Workforce Productivity Compass", margin, ph - 14);
...
const renderModuleSection = async (
  ...
  // page already has the SecKnight Vision branding)
  doc.setFillColor(...BRAND.band);
...
export const generateCombinedDashboardPdf = async ({
  title = "SecKnight Vision Dashboard Report",
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...BRAND.subtle);
    doc.text("SecKnight Vision • Your Workforce Productivity Compass", margin, pageHeight - 14);
  ...
  doc.save(
    fileName ||
      `secknight-vision-dashboard-report-${new Date().toISOString().slice(0, 10)}.pdf`
  );
};
